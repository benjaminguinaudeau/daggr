...
direct and mediated pathways. Uses `dag_fills` and
...
partisan <- dag_ellipse("Partisanship", fill = dag_fills$unmeasured) |>
  place(media, "below", sep = 3)
ses      <- dag_rectangle("SES", fill = dag_fills$instrument) |>
  place(partisan, "left", sep = 3)

ggdiagram(font_size = 14) +
  media + framing + opinion + turnout + partisan + ses +
  dag_edges(
    dag_edge(media, framing),
    dag_edge(framing, opinion),
    dag_edge(opinion, turnout),
    dag_edge(media, opinion, arc_bend = -0.3),
    dag_edge(partisan, media),
    dag_edge(partisan, opinion),
    dag_edge(partisan, turnout),
    dag_edge(ses, media),
    dag_edge(ses, partisan),
    dag_edge(ses, turnout)
  ) +
  theme_dag()
```
...
exposure-induced confounders. This example uses `dag_fills` for semantic
colors and
[`dag_edges()`](https://benjaminguinaudeau.github.io/daggr/reference/dag_edges.md)
to define all connections in a single block.
