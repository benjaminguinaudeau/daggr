...
Instead of repeating hex codes like `"#708090"` throughout your DAG, use the
built-in `dag_fills` palette. It provides autocomplete-friendly names for common
variable types:

```{r fills-demo, eval = FALSE}
dag_fills$treatment   # "#2E7D32" (green)
dag_fills$outcome     # "#C62828" (red)
dag_fills$unmeasured  # "#708090" (slate gray)
```

```{r fills-example, fig.cap = "Using dag_fills"}
...
For DAGs with many edges, `dag_edges()` lets you define all connections in one
block instead of chaining separate `dag_connect()` calls. Shared defaults
(`resect`, `color`) are specified once; per-edge overrides go inside each
`dag_edge()`.

```{r batch-edges, fig.cap = "Mediation DAG with dag_edges()", fig.width = 8, fig.height = 5}
