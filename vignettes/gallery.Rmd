...
vignette: >
  %\VignetteIndexEntry{DAG Gallery}
...
features: mixed node shapes, linetypes, annotations, colors, curved edges, and
complex layouts.

## DGP vs Measurement

Two conceptual layers — the data-generating process (causal truth) and what we
actually observe. Solid arrows are causal; dashed arrows are observed
associations.
...
A complex DAG mixing ellipses (latent variables) and rectangles (constructed
indicators), with different linetypes encoding the type of relationship.

```{r polsci, fig.height = 7}
...
Conditioning on a post-treatment variable can induce selection bias. Here,
**S** (selection into the sample) is caused by both treatment and outcome,
creating a collider structure.

```{r selection-bias, fig.height = 5}
...
The parallel trends assumption visualized. **Group** (treatment vs control) and
**Time** (pre vs post) jointly determine the outcome. The unobserved
counterfactual is what would have happened to the treated group absent
treatment.
...
The running variable determines treatment assignment via a threshold. Potential
confounders affect the outcome but cannot manipulate the running variable
...
A multi-step model of media influence on political behavior, with both direct
and mediated pathways. Uses `dag_fills` and `dag_edges()` to keep the code
compact.

```{r media, fig.height = 7}
...
colors and `dag_edges()` to define all connections in a single block.

```{r mediation, fig.height = 7}
...
When variables are measured with error, the observed indicators (rectangles)
are noisy reflections of the true latent constructs (ellipses).

```{r measurement-error, fig.height = 7}
# Latent variables
...
In network or spatial settings, one unit's treatment can affect another unit's
outcome — a violation of SUTVA.

```{r spillover, fig.height = 5}
